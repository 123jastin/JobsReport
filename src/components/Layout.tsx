    ...
    <div className="min-h-screen bg-bg-deep flex flex-col overflow-x-hidden pt-16">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-16 bg-bg-surface/90 backdrop-blur-md border-b border-white/10 px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          {/* 🍔 Three bars menu button near search on mobile/all screens */}
          <button 
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            ...
            {/* Quick status login controller in mobile drawer */}
              ...
              {isAdmin ? (
                <div className="flex items-center justify-between bg-blue-600/10 p-3 rounded-2xl border border-blue-500/20">
                  ...
                  <button 
                    ...
                    className="p-1 px-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[10px] font-bold uppercase rounded-lg transition-colors"
                  >
                    Logout
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full p-3 rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 font-bold text-stone-100 font-mono text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg"
                >
                  <LogIn size={14} />
                  Authenticate Admin
                </button>
      ...
      {/* 🖥️ Login Authentication Modal/Pop-up (Accessible Globally) */}
      <AnimatePresence>
        {isLoginModalOpen && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            >
              <button 
                ...
                className="absolute top-4 right-4 p-1.5 rounded-full hover:bg-white/5 text-gray-500 hover:text-white transition-colors"
              >
                <X size={16} />
              </button>
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-3 pt-2">
                <input 
                  ...
                  placeholder="Decrypt passcode Key (try: admin)"
                  className="w-full bg-stone-900 border border-white/10 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-blue-500 transition-colors text-center font-mono placeholder:text-gray-650"
                  required
                />
                ...
                <button 
                  type="submit"
                  className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-xs uppercase tracking-widest transition-colors shadow-lg"
                >
                  DECRYPT KEY NODE
                </button>
              </form>
