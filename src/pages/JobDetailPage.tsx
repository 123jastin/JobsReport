...
export default function JobDetailPage() {
    ...
    async function loadJob() {
      try {
        ...
        if (res.ok) {
          ...
          // 🔥 Find job by matching slug directly (MarketPage now uses API slugs)
          let found = allJobsList.find((j: any) => j.slug === jobId);
          
          // Fallback: URL contains job ID
          if (!found) {
            found = allJobsList.find((j: any) => jobId?.includes(j.id));
          }
          
          // Fallback: URL ends with job ID
          if (!found) {
            found = allJobsList.find((j: any) => jobId?.endsWith(j.id));
          }
  ...
  const expiredSchema = isExpired ? { "@context": "https://schema.org", "@type": "WebPage", "name": `${job.title} at ${job.company} (Expired)`, "description": `This job posting for ${job.title} at ${job.company} has expired.`, "url": jobUrl } : null;
