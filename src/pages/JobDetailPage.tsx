    ...
    <div className="min-h-screen bg-black text-white">
          ...
          <div className="min-w-0">
            ...
            {job.companyWebsite ? (
              ...
              >
                ...
                {(() => { 
                  try { 
                    return new URL(job.companyWebsite).hostname.replace('www.', ''); 
                  } catch { 
      ...
      <div className="sticky bottom-0 z-40 bg-black/95 backdrop-blur border-t border-white/5 px-4 py-3 space-y-2">
        ...
        {job.url && !isExpired && !hasWhatsApp && (
          <p className="text-[10px] text-gray-500 text-center truncate px-4">
            ...
            {isEmailLink ? job.url.replace('mailto:', '') : (() => {
              try {
                return new URL(job.url).hostname.replace('www.', '') + new URL(job.url).pathname;
              } catch {
                return job.url;
              }
            })()}
        ...
        {hasWhatsApp && !isExpired ? (
          ...
          <button 
            disabled 
            className="w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 bg-red-500/10 text-red-400 cursor-not-allowed"
          >
            🚫 Application Closed
          </button>
        ) : job.url ? (
          ...
          <button 
            disabled 
            className="w-full py-3.5 rounded-xl font-bold text-sm bg-white/5 text-gray-600 cursor-not-allowed"
          >
                ...
                >
                  {isPDF ? (
                    <div className="w-full h-full flex flex-col items-center justify-center bg-red-900/20 p-2">
                      ...
                      <span className="text-[7px] text-gray-400 mt-0.5 text-center truncate w-full">
                    ...
                    <img 
                      ...
                      alt={img.seoTitle || img.name || 'Attachment'} 
      ...
      {hasDescription && (
        <div className="px-4 py-6 border-b border-white/5">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 flex items-center gap-2">
            <FileText size={16} className="text-blue-400" /> Job Description
          </h3>
          <div 
      ...
      {relatedJobs.length > 0 && (
        <div className="px-4 py-8 border-t border-white/5">
          ...
          <p className="text-xs text-gray-500 mb-4">
            {isExpired 
              ? 'This job has expired. Here are similar active opportunities.' 
        ...
        <div className="fixed inset-0 z-50 bg-black flex flex-col">
          <div className="flex items-center justify-between px-4 h-14 bg-black/90 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <a 
                ...
                download 
                className="px-3 py-1.5 bg-white/10 text-white text-xs rounded-lg flex items-center gap-1"
              >
                <Download size={12} />
              </a>
              <button 
          ...
          <div className="flex-1 flex items-center justify-center p-4">
            {viewerFiles[viewerIndex]?.type === 'pdf' || viewerFiles[viewerIndex]?.name?.endsWith('.pdf') ? (
              ...
              <img 
                src={viewerFiles[viewerIndex]?.url} 
