...
export default function CompaniesPage() {
  ...
  const getCompanyJobs = (companyName: string) => {
    return jobs.filter(job => 
      ...
      // Active jobs first, then expired
                ...
                <div className="flex-1">
                  ...
                  {selectedCompany.url && (
                    ...
                    >
                      ...
                      {(() => {
                        try {
                          return new URL(selectedCompany.url).hostname.replace('www.', '');
                        } catch {
            ...
            <div>
              ...
              {getCompanyJobs(selectedCompany.name).length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {getCompanyJobs(selectedCompany.name).map((job) => (
                    <Link
                      key={job.id}
                      ...
                      className={`block p-5 rounded-2xl border transition-all group ${
                        job.active
                          ? 'bg-white/[0.01] border-white/5 hover:bg-white/[0.03] hover:border-blue-500/30'
                    ...
                    >
                      ...
                      {job.expiresAt && !job.active && (
                        <div className="mt-2 text-[9px] text-amber-400 font-mono">
                          Expired: {job.expiresAt}
                        </div>
                      )}
                    </Link>
                  ))}
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-14 h-14 rounded-2xl bg-white/[0.02] border border-white/10 overflow-hidden flex items-center justify-center shrink-0">
                      {company.logoUrl ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-blue-600 to-violet-600 flex items-center justify-center text-white font-bold text-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {company.url && (
                        <span className="text-[10px] text-gray-500 font-mono truncate block mt-1">
                          {(() => {
                            try {
                              return new URL(company.url).hostname.replace('www.', '');
                            } catch {
                              return '';
                            }
                          })()}
            ...
            {filteredCompanies.length === 0 && (
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-gray-500 text-sm font-mono">
                  {searchTerm ? 'No companies found matching your search.' : 'No companies listed yet.'}
                </p>
              </div>
            )}
          </div>
