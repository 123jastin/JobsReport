  ...
  try {
    ...
    const jobResult = await DB.prepare(`
      SELECT j.id, j.title, j.description, j.job_category, j.industry, j.employment_type, j.workplace_type,
        ...
        r.name as role, c.name as company, c.logo_url, c.website, c.description as company_description,
    ...
    const relatedResult = await DB.prepare(`
      SELECT j.id, j.title, r.name as role, c.name as company, c.logo_url, j.location, j.salary, j.salary_min, j.salary_max, j.salary_currency, j.posted_at, j.expires_at, j.is_active
      FROM jobs j JOIN roles r ON j.role_id = r.id JOIN companies c ON j.company_id = c.id
    ...
    const relatedJobs = relatedResult.results.map((rj: any) => {
      ...
      return {
        id: rj.id, title: rj.title, role: rj.role, company: rj.company, logoUrl: rj.logo_url || '',
    ...
    return new Response(JSON.stringify({
      ...
      images: images,           // 🔥 MUST BE HERE
      relatedJobs: relatedJobs  // 🔥 MUST BE HERE
    }), {
