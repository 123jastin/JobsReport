...
export const onRequestGet = async (context: any) => {
  ...
  try {
    const { results: jobs } = await DB.prepare(`
      SELECT id, title FROM jobs ORDER BY updatedAt DESC
    `).all();
  ...
  try {
    const { results: reports } = await DB.prepare(`
      SELECT id, title, slug FROM reports ORDER BY updatedAt DESC
    `).all();
  ...
  try {
    const { results: roles } = await DB.prepare(`
      SELECT DISTINCT role FROM jobs WHERE role IS NOT NULL AND role != '' LIMIT 50
    `).all();
