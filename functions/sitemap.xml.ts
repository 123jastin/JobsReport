...
export const onRequestGet = async (context: any) => {
  ...
  try {
    ...
    console.error('Sitemap countries error:', err);
  }

  // ========== CATEGORY PAGES ==========
  try {
    const { results } = await DB.prepare(`
      SELECT DISTINCT job_category as name 
      FROM jobs 
      WHERE job_category != '' 
        AND job_category != 'Other' 
        AND is_active = 1
      LIMIT 200
    `).all();
  ...
  try {
    ...
    for (const job of results || []) {
      const slug = (job.title as string).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  ...
  // ========== REGION PAGES (FIXED - Uses job_id JOIN with loc- prefix) ==========
  try {
    const { results } = await DB.prepare(`
      SELECT DISTINCT 
    ...
    console.error('Sitemap regions error:', err);
  }
  ...
  try {
    ...
    console.error('Sitemap roles error:', err);
  }
